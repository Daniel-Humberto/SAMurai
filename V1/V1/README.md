...
**SAM 2 Live v6** representa un rediseño integral enfocado en la **transmisión en vivo (broadcast)** y el **análisis deportivo de alta fidelidad**. Mediante una arquitectura completamente desacoplada con hilos de procesamiento de video e intercambios síncronos sobre WebSockets, el sistema procesa video a alta tasa de frames sin latencia acumulativa.
...
graph TD
    %% Componentes
    UI["💻 Interfaz Web (static/index.html)"]
    WS["🔌 WebSockets Loop (FastAPI)"]
    Cap["📹 VideoCaptureThread (OpenCV)"]
    Queue["📥 Cola Deslizante (maxsize=3)"]
    State["🧠 Gestor de Estado (state.py)"]
    Seg["🔬 Segmenter API (model.py)"]
    Predictor["🤖 SAM2VideoPredictor (PyTorch)"]
    CSV["📄 Telemetría (trajectories.csv)"]
    Rec["🎥 VideoWriter (MP4 Output)"]
    ...
    Seg <-->|Memory Tensors & Pruning| Predictor
...
*   El [Segmenter](file:///home/humbert/Docker/SAM%202/V5/app/model.py) simula un directorio base en disco de memoria compartida (`/dev/shm/sam2_live`) que contiene un frame ficticio de 10x10 píxeles. Esto satisface el proceso de validación estructural inicial del método `init_state` del `SAM2VideoPredictor` de Facebook.
...
El sistema migró de un único objeto a una estructura asociativa en [InferenceState](file:///home/humbert/Docker/SAM%202/V5/app/state.py).
...
*   **Propagación Conjunta:** Al llamar a `track_step` en [model.py](file:///home/humbert/Docker/SAM%202/V5/app/model.py), el predictor calcula las máscaras de todos los objetos registrados en paralelo utilizando los logits del banco de memoria unificado de SAM 2.
...
2.  **Fragmentación CUDA:** Se carga la variable de entorno `PYTORCH_CUDA_ALLOC_CONF="expandable_segments:True"` al inicializar la aplicación en [main.py](file:///home/humbert/Docker/SAM%202/V5/app/main.py) para que PyTorch expanda dinámicamente los bloques de memoria sin fragmentar la memoria virtual de la GPU.
...
Los datos geométricos de cada objeto se registran fotograma a fotograma en [outputs/trajectories.csv](file:///home/humbert/Docker/SAM%202/V5/app/main.py#L31). El archivo incluye las siguientes columnas:
...
*   [sam2_live.sh](file:///home/humbert/Docker/SAM%202/V5/sam2_live.sh): Script controlador Bash. Administra el ciclo de vida del contenedor Docker, monta cámaras de video `/dev/video*`, crea directorios y descarga checkpoints del modelo.
*   [Dockerfile](file:///home/humbert/Docker/SAM%202/V5/Dockerfile): Imagen Docker basada en PyTorch Runtime con CUDA 12.4. Instala dependencias del sistema (FFmpeg, OpenCV) y compila SAM 2 desde el repositorio oficial de Meta.
*   [app/main.py](file:///home/humbert/Docker/SAM%202/V5/app/main.py): Orquestador del servidor web ASGI (FastAPI), bucle de comunicación WebSocket, e hilo de ingesta asíncrona de video `VideoCaptureThread`.
*   [app/model.py](file:///home/humbert/Docker/SAM%202/V5/app/model.py): Encapsulador de la API de SAM 2. Controla la inicialización de `SAM2VideoPredictor`, la poda de memoria temporal de embeddings y la generación automática.
*   [app/state.py](file:///home/humbert/Docker/SAM%202/V5/app/state.py): Almacenamiento thread-safe del estado de la aplicación. Maneja objetos múltiples, configuraciones de renderizado y cálculo de estadísticas.
*   [app/config.py](file:///home/humbert/Docker/SAM%202/V5/app/config.py): Variables de entorno globales del servidor y del modelo (Checkpoint paths, FPS Cap, Puertos).
*   [static/index.html](file:///home/humbert/Docker/SAM%202/V5/static/index.html): Cliente web desarrollado en HTML5/JS moderno con estética dark glassmorphism, controles interactivos, gestor de objetos y tabla de telemetría dinámica.
...
*   **GPU:** Tarjeta gráfica NVIDIA con soporte CUDA (mínimo 6GB VRAM para el checkpoint Tiny).
...
3.  **Selecciona la Opción `1` (Build & Launch):**
    El script creará automáticamente las carpetas `inputs/`, `outputs/` y `checkpoints/`, descargará el modelo Hiera Tiny de Meta, construirá la imagen Docker e iniciará el contenedor con acceso a la GPU y a los dispositivos de cámara `/dev/video0`, `/dev/video1`, y `/dev/video2`.
